...
export function databaseProxy ({ sql, params, method, callback, errCallback }) {
    let request = {
        requestId,
        sql,
        params,
...
export function databaseProxyServerInit (callback = NOOP) {
    let url = `ws://192.168.6.233:12352`;
    Taro.showModal({ title:'开启 databaseProxyServer 中……' });
    my.connectSocket({
        url: url,
        data: '1111',
        success: () => {
            Taro.showModal({ title:'开启 databaseProxyServer 成功' });
            console.log('正在打开websocket', url);
        },
        fail: (err) => {
    ...
    const onMessage = function (msg_event) {
        let data = JSON.parse(msg_event.data);
        let request = JSON.parse(data.request);
        if (request.category == 'DATABASE') {
            doSql({
                sql: request.sql,
                params: request.params,
                method: request.method,
                callback: (rsp) => {
                    let response = {...data,response: rsp, type:'SUCCESS'}
                    my.sendSocketMessage({
                        data: JSON.stringify(response),
                    });
                },
                errCallback: (rsp) => {
                    let response = {...data,response: rsp, type:'ERROR'}
                    my.sendSocketMessage({ data: JSON.stringify(response) });
                },
            });
        }
    };
    ...
    const onOpen = (res) => {
        Taro.showModal({ title:'连接 databaseProxyServer 成功' });
        console.log('连接 databaseProxyServer 成功', res);
        if (databaseProxyDeferred) {
            databaseProxyDeferred.resolve();
        }
        my.offSocketOpen(onOpen);
        my.onSocketClose(onClose);
    ...
    const onError = (res) => {
        Taro.showToast({ title: '连接 databaseProxyServer 失败' + JSON.stringify(res) });
        my.offSocketError(onError);
    };

    const onClose = (res) => {
        Taro.showModal({ title:'databaseProxyServer 关闭' });
        console.log('连接已关闭！', res);
        databaseProxyDeferred = null;
        my.offSocketMessage(onMessage);
        my.offSocketClose(onClose);
    };
 ...
 * @returns {boolean}
 */
export function getDatabaseProxyEnabled () {
    const { databaseProxyMode } = getSettings().proxy;
    return databaseProxyMode == "on";
}
